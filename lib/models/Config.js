...
var Config = function () {
    function Config(cwd, configFile) {
        ...
        this._config = extend({
            ...
            plugins: [require('../plugins/extTemplatedPathPlugin.js'), require('../plugins/requireModulePlugin.js'), require('../plugins/hashPlaceholderPlugin.js'), new webpack.HashedModuleIdsPlugin(), new CaseSensitivePathsPlugin(), new extend(HappyPack({
                loaders: [{
                ...
                if (nextConfig.resolve && nextConfig.resolve.alias) {
                    ...
                    (0, _keys2.default)(alias).map(function (key) {
                        ...
                        if (key.indexOf('$') !== key.length - 1 && /^\/.+/.test(alias[key]) && isRelativePath) {
                            alias[key] = normalize(sysPath.join(_this._config.cwd, alias[key]));
                        }
                    });
