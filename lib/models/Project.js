        ...
        value: function readConfig(options) {
            ...
            if (this.check()) {
                var userConfig = {
                    cwd: this.cwd,
                    _manager: Manager,
                    setConfig: function setConfig(setFun) {
                        var currentConfig = _this.config.getConfig(),
                            nextConfig = setFun && setFun(currentConfig) || {};

                        if (nextConfig.context && !sysPath.isAbsolute(nextConfig.context)) {
                            nextConfig.context = sysPath.resolve(_this.cwd, nextConfig.context);
                        }
        ...
        value: function fixCss() {
            ...
            for (var key in entries) {
                ...
                if (cssExtNames.indexOf(extName) > -1) {
                    (function () {
                        var requireFilePath = entries[key] = './' + sysPath.join(contextPathRelativeToCwd, '/.cache', entry + '.js'),
