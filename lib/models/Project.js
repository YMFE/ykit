    ...
    (0, _createClass3.default)(Project, [{
        ...
        value: function readConfig() {
            ...
            if (ykitConfigFile && ykitConfigFile.config) {
                var ykitJSConfig = typeof ykitConfigFile.config === 'function' ? // 兼容以前从 options 传进去 ExtractTextPlugin
            ...
            */
            ...
            function handleExportsConfig(exportsConfig, options) {
                ...
                this.config.setCompiler(exportsConfig.modifyWebpackConfig, localConfig, this._getCurrentEnv());
        ...
        value: function fixCss() {
            ...
            for (var key in entries) {
                ...
                if (cssExtNames.indexOf(extName) > -1) {
                    (function () {
                        var entryStr = sysPath.join(contextPathRelativeToCwd, YKIT_CACHE_DIR, entry + '.js');
                        if (!entryStr.startsWith('./') && !entryStr.startsWith('.')) {
                            entryStr = './' + entryStr;
                        }
                        var requireFilePath = entries[key] = entryStr;

                        var cacheFilePath = sysPath.join(config.context, requireFilePath);
