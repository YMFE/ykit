    ...
    _createClass(Project, [{
            ...
            if (this.check()) {
                ...
                var _ret2 = function () {
                    var globalConfigs = Manager.readRC().configs || [],
                        ...
                        cwd: _this2.cwd,
                        _manager: Manager,
                        setConfig: _this2.config.setCompiler.bind(_this2.config), // 兼容旧api
                        setCompile: _this2.config.setCompiler.bind(_this2.config), // 兼容旧api
                        setCompiler: _this2.config.setCompiler.bind(_this2.config),
                        setExports: _this2.config.setExports.bind(_this2.config),
                        setGroupExports: _this2.config.setGroupExports.bind(_this2.config),
                        setSync: _this2.config.setSync.bind(_this2.config),
                        setCommands: _this2.setCommands.bind(_this2),
                        setEslintConfig: _this2.setEslintConfig.bind(_this2),
                    ...
                    _this2.plugins.map(function (pluginName) {
                        ...
                        if (fs.existsSync(pluginPath)) {
                            ...
                            if (_module && _module.config) {
                                _module.config.call(userConfig, options, _this2.cwd);
                            }
                            ...
                            if (item) {
                                ...
                                _this2.ignores.push(Manager.loadIgnoreFile(item.path));
                    ...
                    extend(true, userConfig.eslintConfig, Manager.loadEslintConfig(_this2.cwd));
                    _this2.ignores.push(Manager.loadIgnoreFile(_this2.cwd));
                    ...
                    if (configMethod) {
                            ...
                            if (exports) {
                                exports = exports.filter(function (item) {
                                    if ((typeof item === 'undefined' ? 'undefined' : _typeof(item)) === 'object') {
                                        _this3.config.setGroupExports(item.name, item.export);
                                        return false;
                                    } else {
                                        return true;
                                    }
                        ...
                        if (Array.isArray(configMethod)) {
                            _this2.config.setExports(configMethod);
                        }

                        // 如果传入的是一个处理配置方法或对象
                        if (typeof configMethod.config === 'function') {
                            _handleConfigObj.bind(_this2)(configMethod.config.call(userConfig, options, _this2.cwd));
                        } else if (_typeof(configMethod.config) === 'object') {
                            _handleConfigObj.bind(_this2)(configMethod.config);
                        } else {
                            error(_this2.configFile + ' 设置有误，请参考文档 ' + 'http://ued.qunar.com/ykit/docs-%E9%85%8D%E7%BD%AE.html'.underline);
                            return {
                                v: _this2
                            };
                        }
                    ...
                    var output = _this2.config.getConfig().output;
        ...
        value: function lint(dir, callback) {
            ...
            var files = ['.js', '.yaml', '.yml', '.json', ''].map(function (ext) {
                return path.join(_this4.cwd, '.eslintrc' + ext);
            });
