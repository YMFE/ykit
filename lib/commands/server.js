...
exports.run = function (options) {
    ...
    app.use(function (req, res, next) {
        ...
        // 修改 publicPath 为当前服务
        var dirs = UtilPath.normalize(outputDir).split(sysPath.sep);
        var ouputDir = dirs[dirs.length - 1];
        wpConfig.output.local.publicPath = 'http://' + UtilPath.normalize(sysPath.join('127.0.0.1:' + port, projectName, ouputDir + '/'), false);
                ...
                var middleware = middlewareCache[cacheId] = webpackDevMiddleware(compiler, {
                    quiet: true, reporter: function reporter(_ref) {
                        ...
                        if (stats.endTime - stats.startTime > minDuration) {
                            spinner.text = '\x1b[90m' + '[' + moment().format(dateFormat) + '] building complete in ' + (stats.endTime - stats.startTime) + 'ms.';
