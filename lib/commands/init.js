...
exports.run = function (options) {
	...
	if (typeof process.argv[3] === 'string') {
		var checkConfigPkg = function checkConfigPkg(callback, packageName, registry) {
			if (!isInitReady) {
				var timeout = void 0;
				var child = shell.exec('npm view ' + packageName + ' --registry http://registry.npm.' + registry, {
					silent: true
	...
	function createConfigFile(callback, configPkgName) {
		var pluginName = configPkgName.match(/ykit-config-([^\-]+)/)[1];
		...
		if (!UtilFs.fileExists('./' + configFileName)) {
			...
			var stream = fs.createReadStream(configFilePath).pipe(replaceStream('@PROJECT_NAME', projectName)).pipe(replaceStream('@PLUGIN_NAME', pluginName)).pipe(writeStream);
