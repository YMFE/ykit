            ...
            projectList.forEach(project => {
                if(project.alias && project.alias.length) {
                    project.alias.forEach( alias => {
                        // console.log(`rewrite ${alias} -> ${project.baseName}`);
                        var aliasReg = new RegExp(`\^/${alias}$|\^/${alias}/`);
                        if(aliasReg.test(req.url)) {
                            // "X"-prefix 过期了
                            req.setHeader('YKIT_ALIAS', alias);
                            req.setHeader('YKIT_PROJECT_NAME', project.baseName);

                            req.url = req.url.replace(, `/${project.baseName}/`);
                        }
                    });
                }
            });
