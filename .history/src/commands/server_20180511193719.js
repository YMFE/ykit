...
exports.run = (options) => {
    ...
    app.use((req, res, next) => {
        ...
        console.log('projectInfo', projectList);
        
        if(projectList && projectList.length) {
            projectList.forEach(project => {
                if(project.alias && project.alias.length) {
                    project.alias.forEach( alias => {
                        console.log(`replace ${alias} -> ${project.baseName}`, req.url);
                        req.url = req.url.replace(new RegExp(`/${alias}$|/${alias}/$|${alias}/prd/`), `/${project.baseName}/prd/`);
                        console.log(req.url);
                    });
                }
            });
        }
        console.log('projectinfo cost ', Date.now() - st);
        next();
    });
    ...
    function getCurrentProjects() {
        ...
        var isProjectRoot = UtilFs.fileExists(sysPath.join(cwd, 'package.json'));

        if(isProjectRoot) {// 项目根目录访问
    ...
    function _getProjectAlias(projectBase) {
        var isProjectRoot = UtilFs.fileExists(sysPath.join(projectBase, 'package.json'));
